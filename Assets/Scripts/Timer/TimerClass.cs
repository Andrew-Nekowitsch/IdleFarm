...
/// </summary>
    public class TimerClass
    {
        ...
        public static TimerClass Register(float duration, Action onComplete, Action<float> onUpdate = null,
            bool isLooped = false, bool useRealTime = false, MonoBehaviour autoDestroyOwner = null)
        {
            // create a manager object to update all the timers if one does not already exist.
            if (TimerClass._manager == null)
            {
                TimerManager managerInScene = Object.FindObjectOfType<TimerManager>();
                if (managerInScene != null)
                {
                    TimerClass._manager = managerInScene;
                }
                else
                {
                    GameObject managerObject = new GameObject { name = "TimerManager" };
                    TimerClass._manager = managerObject.AddComponent<TimerManager>();
                }
            }

            TimerClass timer = new TimerClass(duration, onComplete, onUpdate, isLooped, useRealTime, autoDestroyOwner);
            TimerClass._manager.RegisterTimer(timer);
            return timer;
        }
        ...
        public static void Cancel(TimerClass timer)
        {
            if (timer != null)
            {
                timer.Cancel();
        ...
        public static void Pause(TimerClass timer)
        {
            if (timer != null)
            {
                timer.Pause();
        ...
        public static void Resume(TimerClass timer)
        {
            if (timer != null)
            {
                timer.Resume();
        ...
        public static void CancelAllRegisteredTimers()
        {
            if (TimerClass._manager != null)
            {
                TimerClass._manager.CancelAllTimers();
            }
        ...
        public static void PauseAllRegisteredTimers()
        {
            if (TimerClass._manager != null)
            {
                TimerClass._manager.PauseAllTimers();
            }
        ...
        public static void ResumeAllRegisteredTimers()
        {
            if (TimerClass._manager != null)
            {
                TimerClass._manager.ResumeAllTimers();
            }
        ...
        private TimerClass(float duration, Action onComplete, Action<float> onUpdate,
        ...
        /// <summary>
        /// Manages updating all the <see cref="TimerClass"/>s that are running in the application.
        /// This will be instantiated the first time you create a timer -- you do not need to add it into the
        /// scene manually.
        /// </summary>
        private class TimerManager : MonoBehaviour
        {
            private List<TimerClass> _timers = new List<TimerClass>();

            // buffer adding timers so we don't edit a collection during iteration
            private List<TimerClass> _timersToAdd = new List<TimerClass>();

            public void RegisterTimer(TimerClass timer)
            {
            ...
            {
                foreach (TimerClass timer in this._timers)
                {
                    timer.Cancel();
                }

                this._timers = new List<TimerClass>();
                this._timersToAdd = new List<TimerClass>();
            }
